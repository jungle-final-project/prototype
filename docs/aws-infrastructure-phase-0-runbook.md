...
현재 CloudFront는 Web과 API가 분리되지 않았으며 모든 요청을 같은 EC2 Origin으로 전달한다. 이후 문서에서는 이 기존 통합 EC2를 `Blue`라고 부른다.

## Phase 0 이후 확정된 Blue/Green 원칙

- 현재 `buildgraph-demo-ec2`와 `compose.prod.yaml`은 Blue 운영 기준선으로 유지한다.
- 새 API 전용 EC2를 Green으로 별도 생성하고 `nginx`, `api`, `xgb-reranker`만 배포한다.
- Green 검증 중에는 Blue EC2, Compose, Docker Volume, CloudFront Origin을 변경하거나 중지하지 않는다.
- Blue와 Green은 서로 다른 EC2이므로 Nginx `80`과 API 컨테이너 `8080`을 동일하게 사용한다.
- CloudFront 전환 후에도 롤백 대기 기간이 끝날 때까지 Blue를 실행한다.
- Green은 Blue의 기존 `buildgraph-demo-ec2-sg`를 공유하므로 SSH 22 규칙을 일시적으로 상속하지만, Green 접속과 배포에는 Session Manager와 SSM Run Command를 사용한다.
