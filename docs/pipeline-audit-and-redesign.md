...
| P0-9 데모 토큰 하드코딩 제거 | 보안 | ✅ 완료 — `agent.demo-activation-token` 프로퍼티 게이트 → **dev 병합(2026-07-06)으로 데모 경로 자체가 DB 발급 activation token 체계로 대체되어 프로퍼티도 제거됨** | 6d55466 → dev 병합 |
...
**P0-9. demo-agent-activation-token 하드코딩 제거 [보안]** — ✅ 구현됨(6d55466, 프로퍼티 게이트). *이후 dev 병합(2026-07-06)으로 등록이 DB 발급 activation token 검증(`agent_activation_tokens`, V69)으로 전환되어 데모 토큰 경로·프로퍼티가 완전히 제거됨 — 이 항목의 보안 의도가 근본적으로 충족.*
