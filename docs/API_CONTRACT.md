...
- **부품 추천을 부품 목록 패널로 넘기기**: request `uiContext.capabilities`에 `PART_CANDIDATE_PANEL`이 있고 그 턴이 **단일 카테고리** 부품 추천이면 optional `partRecommendation={category, options:[{partId,name,price}]}`를 반환하고, `message`는 상품 나열 대신 한 줄로 줄인다(예: `내부 자산 기준으로 고른 케이스 추천 3개를 부품 목록에 띄웠어요.`). 여러 카테고리를 한꺼번에 물으면(`케이스랑 파워 추천해줘`) 내려주지 않는다 — 패널은 한 번에 한 카테고리만 연다. capability를 선언하지 않은 클라이언트에는 이 필드가 없고 종전대로 말풍선에 TOP 목록과 담기 칩이 실린다(하위 호환). 클라이언트는 `/self-quote?category={category}`로 이동해 패널을 열고 `options` 순서를 목록 맨 위에 올린다. 읽기 전용이며 quote draft는 바꾸지 않는다 — 담기는 사용자가 패널에서 누른다. `quickReplyCommands` 담기 칩은 이 턴에도 그대로 유지한다.
