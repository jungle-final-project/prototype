...
| WebSocket | `/ws/support-chat?mode=user&sessionId={id}` | USER/ADMIN | 4번 | 연결 후 5초 안에 `{ "type": "AUTH", "ticket": "opaque-token" }`. 수신 전용. client `MESSAGE`는 저장하지 않고 `{ "type": "ERROR", "code": "WS_MESSAGE_DISABLED", "retryable": false }` 반환 | `{ "type": "CHAT_UPDATED", "detail": SupportChatSessionResponse }` | `support_chat_rooms` |
...
- 메시지는 REST `POST .../messages` 성공 시에만 저장된다. 저장 후 서버는 해당 상담방에 연결된 모든 WebSocket 세션에 `CHAT_UPDATED`를 push한다. 각 세션은 자신의 모드(user/admin) 기준 unread-safe `SupportChatSessionResponse` snapshot을 받는다.
...
| `POST` | `/api/agent/diagnosis-chat` | AGENT | 4번 | `{ "message": "AS 접수해야 해?", "diagnosis": { "summaryText": "Driver reset repeated", "recommendedService": "REMOTE_SUPPORT", "recommendedDecision": "REMOTE_POSSIBLE", "confidence": "HIGH" }, "messages": [{ "role": "user", "content": "게임이 꺼져요" }] }` | `{ "assistantMessage": "먼저 드라이버와 문제 앱 재시작/업데이트부터 확인해 보세요.", "causeCandidates": [], "nextActions": [], "escalation": { "required": false, "recommended": true, "reason": "Remote-checkable signals repeat." }, "ticketDraft": { "symptomSummary": "Driver reset repeated" }, "model": "buildgraph-agent-diagnosis-rule-v1" }` | none. Stateless agent-token chat; does not create `as_tickets`, `agent_log_uploads`, `as_chat_sessions`, or `as_chat_messages`. |
