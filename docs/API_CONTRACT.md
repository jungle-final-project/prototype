...
- Build Chat 의도 분기는 내부 `BuildChatIntentRouter` 결정값을 기준으로 한다. intent는 `SIMULATE_REPLACEMENT`, `BUILD_RECOMMEND`, `LOCATE_BOARD_PART`, `ASK_CLARIFICATION`, `UNSUPPORTED` 5종이다. 서버는 decision의 `confidence`, `sideEffectRisk`, `targetCategory`, `targetCategories`, `partQuery`, `preferredPath`, `cachePolicy`, `semanticConstraintSignature`, `ambiguityReasons`를 내부 로그와 테스트에서만 사용하고 공개 응답 shape에는 노출하지 않는다.
...
| WebSocket | `/ws/pc-agent/diagnosis` | AGENT | 4번 | outbound 연결 후 5초 안에 `{ "type": "AUTH", "agentToken": "raw-agent-token" }`; 서버는 `READY`, `DIAGNOSIS_REQUEST`를 전송하고 Agent는 `DIAGNOSIS_RESPONSE`로 응답 | `READY.detail.deviceId`, `DIAGNOSIS_REQUEST.detail`, `DIAGNOSIS_RESPONSE.status` | API JVM session map |
