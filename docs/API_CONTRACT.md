...
- **점수 근거에서 이어지는 케이스 개선 적용**: 현재 견적에 케이스 장착 여유 또는 명시된 통풍 구성으로 인한 score cap이 있을 때 `현재 부품이 여유 있게 들어가는 케이스 추천해줘`, `통풍형 케이스 추천해줘`를 요청하면 일반 가격 TOP3를 그대로 반환하지 않는다. 서버가 각 ACTIVE 케이스로 현재 견적을 다시 평가해 Tool `FAIL`이 없고, 알려진 GPU·쿨러·PSU 장착 여유가 나빠지지 않으며, 케이스 cap과 최종 종합 점수가 실제로 상승하는 후보 중 1위만 반환한다. 해당 단일 변경안에는 `AUTO_APPLY_VERIFIED_CASE_IMPROVEMENT` badge를 붙이고 프론트가 즉시 한 번 반영한다. 현재 케이스와 장착 수치가 같은 파생 상품은 제외하며, 검증된 개선 후보가 없으면 임의 후보를 만들거나 자동 적용하지 않는다. 현재 케이스가 이미 통풍형이고 실제 제한 원인이 PSU/GPU/쿨러 치수라면 통풍 부족이라고 설명하지 않고 해당 부품의 실제 길이·케이스 허용 길이·여유 mm를 안내한다. `케이스 추천해줘` 같은 일반 추천은 이 자동 적용 예외에 포함하지 않는다.
...
| `AiBuildRecommendationDto` | `badges` | `string[]` | yes | `["DRAFT_EDIT_PREVIEW"]`, 케이스 `size` FAIL 단일 복구안의 `["DRAFT_EDIT_PREVIEW", "AUTO_APPLY_VERIFIED_REPAIR"]`, 또는 검증된 케이스 여유 개선안의 `["DRAFT_EDIT_PREVIEW", "AUTO_APPLY_VERIFIED_CASE_IMPROVEMENT"]` |
