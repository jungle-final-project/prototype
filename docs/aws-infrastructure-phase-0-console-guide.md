...
Phase 0 완료 이후 기존 `buildgraph-demo-ec2`는 Blue 환경으로 확정되었다. 새 API 전용 Green EC2는 이후 Phase에서 별도로 생성하므로 이 문서의 기존 EC2 확인·접속 절차는 모두 Blue 기준선 확인을 의미한다. Green 검증과 CloudFront 전환이 끝날 때까지 Blue를 중지하거나 변경하지 않는다.
