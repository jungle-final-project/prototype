...
|---|---|---|---|
| `/` | 1번 | 2번 | `POST /api/ai/build-chat`, `POST /api/build-graphs/resolve`, `POST /api/parts/compatible-candidates`, `GET /api/quote-drafts/current`(챗봇이 견적 문맥 어휘에서 선조회), `PUT /api/quote-drafts/current/apply-ai-build`, `GET /api/public/home`, `GET /api/recommendations/home-parts`, `POST /api/recommendation-events` |
