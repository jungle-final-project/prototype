...
test('selects a featured recommendation and applies every build part to self quote', async ({ page }) => {
  ...
  await expect(page.getByTestId('slot-GPU').getByText('Home RTX 5070 GPU')).toBeVisible();
  await expect(page.getByTestId('slot-CASE').getByText('Home FRAME 4000D Case')).toBeVisible();
});
