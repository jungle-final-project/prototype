    ...
    <Screen>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_400px]">
        <div className="space-y-5">
          ...
          {request.contact ? <Panel title="선택 작업 연락정보" subtitle="가상 결제가 완료되어 선택 기사에게만 공개된 정보입니다."><div className="grid gap-3 sm:grid-cols-2"><ProfileLine label="수령인" value={request.contact.name} /><ProfileLine label="연락처" value={request.contact.phone} /><ProfileLine label="주소" value={[request.contact.postalCode, request.contact.addressLine1, request.contact.addressLine2].filter(Boolean).join(' ')} /><ProfileLine label="요청사항" value={request.note || '-'} /></div></Panel> : null}
        </div>
...
function TechnicianHeader({ profile }: { profile?: Technician }) {
  return <header className="mb-5 flex flex-col gap-3 border-b border-commerce-line pb-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-black text-brand-blue"><Wrench size={15} /> 외부 파트너 기사</div><h1 className="mt-2 text-3xl font-black text-commerce-ink">기사 포털</h1><p className="mt-2 text-sm text-slate-600">조건에 맞는 조립 요청에 견적서를 제출합니다.</p></div><nav className="flex flex-wrap gap-2"><PortalLink to="/technician" icon={<Store size={15} />} label="요청함" /><PortalLink to="/technician/jobs" icon={<BriefcaseBusiness size={15} />} label="선택된 작업" />{profile ? <span className="inline-flex min-h-10 items-center gap-2 rounded-md bg-emerald-50 px-3 text-xs font-black text-emerald-700"><BadgeCheck size={15} /> {profile.displayName}</span> : null}</nav></header>;
}
...
function CheckboxGroup({ label, values, selected, onChange, labels = {} }: { label: string; values: string[]; selected: string[]; onChange: (values: string[]) => void; labels?: Record<string, string> }) { return <fieldset><legend className="text-xs font-black text-slate-600">{label}</legend><div className="mt-2 flex flex-wrap gap-2">{values.map((value) => <label key={value} className={`cursor-pointer rounded-md border px-3 py-2 text-xs font-black ${selected.includes(value) ? 'border-brand-blue bg-blue-50 text-brand-blue' : 'border-commerce-line text-slate-600'}`}><input type="checkbox" checked={selected.includes(value)} onChange={() => onChange(selected.includes(value) ? selected.filter((item) => item !== value) : [...selected, value])} className="sr-only" />{labels[value] ?? value}</label>)}</div></fieldset>; }
